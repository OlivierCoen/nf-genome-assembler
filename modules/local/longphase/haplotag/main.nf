process LONGPHASE_HAPLOTAG {
    ...
    input:
    tuple val(meta), path(bam), path(bai), path(phased_vcf), path(vcf_index), path(fasta), path(fasta_index), path(phased_sv_vcf), path(sv_vcf_index)

    output:
    tuple val(meta), path("*${prefix}.bam"),                                                                   emit: haplotagged_bam
    tuple val("${task.process}"), val('longphase'), eval('longphase --help | head -1 | sed "s/Version: //g"'), topic: versions

    script:
    def args = task.ext.args ?: ''
    def prefix = task.ext.prefix ?: "${meta.id}"
    def sv_file_arg = phased_sv_vcf && sv_vcf_index ? "--sv-file ${phased_sv_vcf}": ""
    """
