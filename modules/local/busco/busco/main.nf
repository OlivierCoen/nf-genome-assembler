process BUSCO_BUSCO {
    ...
    tuple val(meta), path("*-busco.batch_summary.txt"),                                         emit: mqc_busco_batch_summary
    tuple val(meta), path("short_summary.*.txt"),                                               emit: mqc_busco_short_summaries_txt , optional: true

    path("*-busco.batch_summary.txt"),                                                          topic: mqc_busco_batch_summary
    path("short_summary.*.txt"),                                                                topic: mqc_busco_short_summaries_txt , optional: true
    ...
    mv ${prefix}-busco/batch_summary.txt batch_summary.txt.tmp
    ...
    # fixing batch summary file for MultiQC
    sed '1s/ /_/g' batch_summary.txt.tmp | sed 's/ /\t/g'  > ${prefix}-busco.batch_summary.txt
    """
